...
namespace Catalog.Infrastructure.Messaging.Listener;
...
public sealed class ShowtimeRpcConsumer : BackgroundService
{
    private readonly IServiceProvider _serviceProvider;

    public ShowtimeRpcConsumer(IServiceProvider serviceProvider)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        ...
        // Cola para obtener información de showtimes
        ...
        Console.WriteLine("[CATALOG] ✓ Colas y exchange declarados. Esperando mensajes...");

        // Consumer para obtener información de showtimes
        ...
        getShowtimeConsumer.ReceivedAsync += async (_, ea) =>
        {
            ...
            try
            {
                ...
                if (showtime != null)
                {
                    response = new GetShowtimeResponse(
                        showtime.Id, 
                        $"Película ID: {showtime.MovieId}", // Título genérico ya que no tenemos MovieTitle
                        showtime.StartTime, 
                        showtime.Room); // Usar Room del DTO directamente
                    Console.WriteLine($"[CATALOG] ✓ Showtime encontrado: {response.MovieTitle}");
                }
                else
                {
                    response = new GetShowtimeResponse(request.ShowtimeId, "Película Desconocida", DateTime.Now, "Sala Desconocida");
            ...
            catch (Exception ex)
            {
                ...
                var errorResponse = new GetShowtimeResponse(0, "Error", DateTime.Now, "Error");
        ...
        Console.WriteLine("[CATALOG] ✓ Consumer configurado");

        var tcs = new TaskCompletionSource<bool>();
        ct.Register(s => ((TaskCompletionSource<bool>)s).SetResult(true), tcs);
        await tcs.Task;

        Console.WriteLine("[CATALOG] ✓ Finalizando RPC Consumer");
    }
}
